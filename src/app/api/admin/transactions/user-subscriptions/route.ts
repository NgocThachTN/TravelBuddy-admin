...
const BACKEND_ROUTE = "/api/v1/admin/transactions/user-subscriptions";
...
function buildQueryParams(searchParams: URLSearchParams): Record<string, string> {
  ...
  const packageName = nonEmpty(searchParams.get("packageName"));
  if (packageName) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const e = logAndExtract(err, "GET admin user subscription transactions");
    if (e.status === 404) {
      return NextResponse.json({
        ...
        message: "Backend chưa hỗ trợ endpoint giao dịch mua gói người dùng.",
      });
    }
