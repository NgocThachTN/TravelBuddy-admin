...
type ReportScope = "admin" | "moderation";
...
function ProcessDialog({ report, scope, onClose, onConfirm, loading }: ProcessDialogProps) {
  return (
...
function ProcessForm({
  ...
  // Admin actions vs Moderation actions
  const adminActions: ResolvedActionCode[] = ["None", "Warn", "LockUser", "BanUser", "SuspendPartner", "Refund"];
  const moderationActions: ResolvedActionCode[] = ["None", "Warn", "RemoveContent", "CancelRescueRequest"];
  const availableActions = scope === "admin" ? adminActions : moderationActions;
...
export default function ReportTable() {
  ...
  const [scope, setScope] = useState<ReportScope>("admin");
  ...
  useEffect(() => { setPage(1); }, [debouncedSearch, statusFilter, targetTypeFilter, scope]);
  ...
  const loadReports = useCallback(async (p = page) => {
    try {
      ...
      const fetcher = scope === "admin" ? fetchReports : fetchModerationReports;
  ...
  }, [page, debouncedSearch, statusFilter, targetTypeFilter, scope]);
  ...
  async function handleProcess(payload: ProcessReportPayload) {
    ...
    try {
      ...
      const processor = scope === "admin" ? processAdminReport : processModerationReport;
      await processor(processTarget.reportId, payload);
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex border-b">
          ...
          <button
            ...
            onClick={() => setScope("moderation")}
          >
            Kiểm duyệt (Nội dung & Cứu hộ)
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 border-b p-4">
          ...
          {/* Target Type Filter — changes based on scope */}
