...
import { VEHICLE_CATEGORY_CODES } from "@/types";
...
export default function VehicleCategoryTab() {
  ...
  const [formType, setFormType] = useState<VehicleCategoryTypeCode>("Other");
  ...
  function openEdit(cat: VehicleCategoryDto) {
    setEditing(cat);
    setFormName(cat.name ?? "");
    setFormType((cat.type as VehicleCategoryTypeCode) ?? "Other");
  ...
  async function handleSave() {
    ...
    try {
      ...
      setFormError(null);
      if (editing) {
        await updateVehicleCategory(editing.vehicleCategoryId, {
          name: formName.trim(),
          type: formType,
      ...
      <Card className="border-destructive/20 bg-destructive/5">
        <CardContent className="flex flex-col items-center py-14">
          <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-destructive/10">
            <Car className="h-7 w-7 text-destructive" />
          </div>
    ...
    <>
      <Card className="overflow-hidden py-0">
        <div className="flex items-center justify-between border-b px-5 py-3.5">
          <span className="text-xs text-muted-foreground">{categories.length} loại phương tiện</span>
            ...
            {categories.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="py-12 text-center">
                  <p className="text-sm text-muted-foreground">Chưa có loại phương tiện nào</p>
                </TableCell>
              </TableRow>
            ) : (
              categories
                ...
                .map((cat) => (
                  <TableRow key={cat.vehicleCategoryId} className="group">
      ...
      <Dialog open={dialogOpen} onOpenChange={(v) => { if (!v) setDialogOpen(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editing ? "Chỉnh sửa loại phương tiện" : "Thêm loại phương tiện"}
            </DialogTitle>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={formType} onValueChange={(v) => setFormType(v as VehicleCategoryTypeCode)}>
                <SelectTrigger className="h-9">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {VEHICLE_CATEGORY_CODES.map((code) => (
